...
#The key and url for api access
#this project uses the new york times article search api
...
raw_election_data = new_dir/'raw_election_data.csv'
raw_election_data.touch(exist_ok=True)
...
#creating a fucntion which places the data inputed into its appropriate place in the parameters of a search. This function than takes the data from the api, extracts the total hit count and returns it as the value
...
#using a for loop to go through the list of elections. This for loop calls on the function "search()" to place the appropriate data from the lists into their respective places in the paramaters of a api quary
for i in elections:
...
raw_data.to_csv(raw_election_data, index=False)
...
#using a for loop to extract the data from the dataframe created in the previous step. This loop finds the disparity percentage between articles writen about each canidate and appends it to  two lists expressed in decimal form
...
#creating a second data frame for the data results and saving it to a csv
results = new_dir/'data_results.csv'
results.touch(exist_ok=True)

results_dict = {
    'test' : ['incumbent','total'],
    'average_disparity' : [mean_incumbent, mean_total],
    'standard_deviation' : [stand_dev_incumbent, stand_dev_total],
    'upper_limit_of_normal' : [upper_incumbent, upper_total],
    'lower_limit_of_normal' : [low_incumbent, low_total],
}
...
#writing final results to a txt file
with open (new_dir/'results.txt', 'w') as file:
    file.write(f"The total hits for trump is {trump_hits}.\n")
    file.write(f"The total hits for biden is {biden_hits}.\n")
    file.write(f"The disparity between articles about Biden and Trump is {(disparity_2020*100):.2f}%\n")
    file.write("\n")

    file.write(f"Normal disparity for all elections: {(low_total*100):.2f}% to {(upper_total*100):.2f}%\n")
    file.write(f"Normal disparity for incumbent elections: {(low_incumbent*100):.2f}% to {(upper_incumbent*100):.2f}%\n")
    file.write("\n")
    ...
    if (disparity_2020 > upper_total or disparity_2020 < low_total) and (disparity_2020 > upper_incumbent or disparity_2020 < low_incumbent):
        file.write(f'The disparity between the articles written about trump and biden is greater than two standard deviations way from the mean making it statistically abnormal.')
    elif (disparity_2020 > upper_total or disparity_2020 < low_total) and (low_incumbent <= disparity_2020 <= upper_incumbent):
        file.write(f'The disparity between the articles written about trump and biden is greater than two standard deviations way from the mean for all elections but less than two standard deviations way from the mean for incumbents making the disparity normal')
    else:
        file.write(f'The disparity between the articles written about trump and biden is less than two standard deviations way from the mean making the discrepancy normal.') 
...
with open (new_dir/'results.txt', 'r') as file:
    content = file.read()
